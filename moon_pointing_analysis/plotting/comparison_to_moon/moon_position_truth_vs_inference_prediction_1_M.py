...
print(len(azimuth_prediction))
print(len(azimuth_spline_mpe_ic))
hist = axs.hist2d(azimuth_prediction, azimuth_spline_mpe_ic, bin_number,cmap="viridis")
axs.set_title("Azimuth",fontsize=fs)
axs.set_ylabel('Spline MPE Reconstruction [deg]',fontsize=fs)
axs.set_xlabel("GraphNet Reconstruction [deg]",fontsize=fs)
axs.tick_params(axis='x', labelsize=fs)
axs.tick_params(axis='y', labelsize=fs)
cb = fig.colorbar(hist[3], ax=axs)
...
axs.set_title("Zenith",fontsize=fs)
axs.set_ylabel('Spline MPE Reconstruction [deg]',fontsize=fs)
axs.set_xlabel("GraphNet Reconstruction [deg]",fontsize=fs)
axs.tick_params(axis='x', labelsize=fs)
axs.tick_params(axis='y', labelsize=fs)
