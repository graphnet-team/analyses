...
class RunParameters:
    ...
    InfMuonAmpSplinePath = (
        "/data/sim/sim-new/spline-tables/InfBareMu_mie_abs_z20a10.fits"
    )
    InfMuonProbSplinePath = (
        "/data/sim/sim-new/spline-tables/InfBareMu_mie_prob_z20a10.fits"
    )
    CascadeAmpSplinePath = (
        "/data/sim/sim-new/spline-tables/ems_mie_z20_a10.abs.fits"
    )
    CascadeProbSplinePath = (
        "/data/sim/sim-new/spline-tables/ems_mie_z20_a10.prob.fits"
    )
    PhotonicsTableDir = (
        "/home/jfeintzeig/netuser/Tables/mie/SPICEMie/level2/standard/"
    )
...
def main(params):
    ...
    def takeEvents(frame):
        if MC:
            if "MPEFit" in frame:
                if (
                    frame["MPEFit"].dir.zenith < 1.919862177
                    and frame["MPEFit"].dir.zenith > 0.8726646259971648
                ):
                    return True
                else:
                    return False
            ...
            try:
                if (
                    frame["FilterMask"]["MoonFilter_13"].condition_passed
                    and frame["I3DST"].ndom >= NChCut
                ):
                    return True
                else:
                    return False
    ...
    def compressTruncatedEnergy(frame):
        ...
        for i in range(len(truncKeys)):
            if truncKeys[i] in frame:
                try:
                    truncEn = frame[truncKeys[i]].value
                except:
                    truncEn = frame[truncKeys[i]].energy
            else:
                truncEn = -1
            frame.Delete(truncKeys[i])
            TruncatedEnergyDic[dicKeys[i]] = truncEn
        frame.Put(
            "SplineMPETruncatedEnergy",
            dataclasses.I3MapStringDouble(TruncatedEnergyDic),
        )
...
def parseOptions(parser, params):
    parser.add_option(
        ...
        default="",
        dest="GCDFILE",
        help="complete path to gcd file",
    )
    parser.add_option(
        "-i",
        "--inputfile",
        default=[],
        dest="INPUTFILE",
        help="complete path to input file",
    )
    parser.add_option(
        "-o",
        "--outputdir",
    ...
    params.OutputFile = (
        options.INPUTFILE.split("/")[-1]
        .replace("Level2", "Level3")
        .strip(".bz2")
        .replace("*", "")
        .replace("[0-9]", "")
    )
...
try:
    ...
    class IC86MuonLevel3(ipmodule.IPBaseClass):
        ...
        def __init__(self):
            ...
            self.AddParameter(
                "infile", "files to be processed", params.InputFile
            )

            self.AddParameter(
                "gcdfile", "input gcd file to use", params.GCDFile
            )
        ...
        def Execute(self, stats):
            ...
            print("Infiles:", params.InputFile)
            print(
                "Outfiles:", params.OutputFile, params.HD5File, params.ROOTFile
            )

            params.InfMuonAmpSplinePath = self.GetParameter(
                "InfMuonAmpSplinePath"
            )
            params.InfMuonProbSplinePath = self.GetParameter(
                "InfMuonProbSplinePath"
            )
            params.CascadeAmpSplinePath = self.GetParameter(
                "CascadeAmpSplinePath"
            )
            params.CascadeProbSplinePath = self.GetParameter(
                "CascadeProbSplinePath"
            )
            ...
            params.PhotonicsDriverFile = self.GetParameter(
                "PhotonicsDriverFile"
            )
