...
from .detector import IceCubeDeepCore_IDON
from .dataset import SQLiteDataset_IDON_Tilt, SQLiteDataset_IDON_Tilt_Transformed
...
class Vals_IDON_Tilt(common.Vals):
    def __init__(self, args: common.Args, *, nb_nearest_neighbours: int = 8):
        ...
        self.detector = IceCubeDeepCore_IDON(
        ...
        self.model = Model(
            ...
            optimizer_kwargs={'lr': 1e-03, 'eps': 1e-03},  # 1e-03, 1e-04, 1e-02
            scheduler_class=PiecewiseLinearLR,
            scheduler_kwargs={
                'milestones': [
                    0,
                    len(self.training_dataloader) / 2,  # / 1
                    len(self.training_dataloader) * args.max_epochs,
                ],
    ...
    def __init__(self, args: common.Args, *, nb_nearest_neighbours: int = 8):
        ...
        self.model = Model(
            ...
            scheduler_kwargs={
                'milestones': [
                    0,
                    len(self.training_dataloader) / 2,
                    len(self.training_dataloader) * args.max_epochs,
                ],
                'factors': [1e-2, 1, 1e-02],
            },
