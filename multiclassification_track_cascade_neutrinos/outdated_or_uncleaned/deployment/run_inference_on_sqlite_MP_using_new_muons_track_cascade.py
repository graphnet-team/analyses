...
def main(
    ...
    #RD_selection = pd.read_csv('/groups/icecube/peter/workspace/analyses/multi_classification_on_stop_and_track_muons/plotting/Comparison_RD_MC/dev_lvl3_genie_burnsample_RD_event_numbers.csv').reset_index(drop = True)['event_no'].ravel().tolist()[:1000]
    ...
    archive = output_path
    run_name = "dynedge_trained_on_Peter_Morten_RD_{}_predict_track_cascade_MP_first".format(
        config["target"]
    )
...
#    )
    ...
    # save_results(config["db"], run_name, results, archive, model)
    resultsRD.to_csv(
        output_folder + "/{}_Track_cascade_whole_MP_db.csv".format(config["target"])
    )
...
if __name__ == "__main__":
    ...
    model_path = "/groups/icecube/petersen/GraphNetDatabaseRepository/multi_classification_track_cascade_neutrino/using_MP_lvl3/trained_models/osc_next_level3_v2/dynedge_track_mu_Track_cascade_MP_data_SplitInIcePulses_on_equal_track_cascade_neutrinos_test/dynedge_track_mu_Track_cascade_MP_data_SplitInIcePulses_on_equal_track_cascade_neutrinos_test_state_dict.pth"
