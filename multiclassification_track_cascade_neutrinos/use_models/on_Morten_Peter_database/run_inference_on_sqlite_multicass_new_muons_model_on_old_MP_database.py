...
def main(
    ...
    archive = output_path
    run_name = "dynedge_trained_on_New_muon_Peter_RD_{}_predict_on_Morten_Peter_database".format(
        config["target"]
    )
...
#    )
    ...
    # save_results(config["db"], run_name, results, archive, model)
    resultsRD.to_csv(
        output_folder + "/{}_Multiclass_try_2_on_old_db_trained_on_new_muons.csv".format(config["target"])
    )
