...
def main(
    ...
    archive = output_path
    run_name = "dynedge_trained_on_Peter_Morten_RD_{}_predict_new_muon_test_set".format(
        config["target"]
    )
...
#    )
    ...
    # save_results(config["db"], run_name, results, archive, model)
    resultstest.to_parquet(
        output_folder + "/{}_New_muon_test_set.parquet".format(config["target"])
    )
    # resultstest.to_csv(
    #     output_folder + "/{}_New_muon_test_set.csv".format(config["target"])
    # )
# Main function call
if __name__ == "__main__":
    ...
    model_path = "/groups/icecube/petersen/GraphNetDatabaseRepository/multi_classification_track_cascade_neutrino/using_MP_lvl3/trained_models/osc_next_level3_v2/MP_data_zenith_1_mill_even_track_cascade_attempt2_test_set/MP_data_zenith_1_mill_even_track_cascade_attempt2_test_set_state_dict.pth"
