...
from graphnet.training.loss_functions import CrossEntropyLoss, EuclideanDistanceLoss
...
from graphnet.models.task.reconstruction import MulticlassClassificationTask,PositionReconstruction
...
parser.add_argument(
    ...
    default="/groups/icecube/petersen/GraphNetDatabaseRepository/multi_classification_track_cascade_neutrino/using_new_muons_Peter_database/trained_models/" ,
    # required=True,
)
parser.add_argument(
    "-p",
...
parser.add_argument(
    ...
    type=str,
    help="<required> the name for the model. [str]",
    default='Peter_new_muon_db_SplitInIcePulses_position'#"last_one_lvl3MC_SplitInIcePulses_21.5_mill_equal_frac"
    # required=True,
)
parser.add_argument(
    "-a",
...
def main():
    ...
    train_selection = pd.read_csv('/groups/icecube/petersen/GraphNetDatabaseRepository/osc_next_database_new_muons_peter/train_val_test_split/vertex_position/only_neutrinos_from_multiclass_train_event_nos.csv').reset_index(drop = True)['event_no'].ravel().tolist()
    validation_selection = pd.read_csv('/groups/icecube/petersen/GraphNetDatabaseRepository/osc_next_database_new_muons_peter/train_val_test_split/vertex_position/only_neutrinos_from_multiclass_val_event_nos.csv').reset_index(drop = True)['event_no'].ravel().tolist()
    ...
    #train_selection = train_selection[:10000]
    #validation_selection = validation_selection[:1000]
    ...
    results_test = get_predictions(
        ...
        additional_attributes=["position_x","position_y", "position_z", "event_no"],
    )
    save_results(config["db"], config["run_name"] + '_test_set', results_test, config["archive"], model)
