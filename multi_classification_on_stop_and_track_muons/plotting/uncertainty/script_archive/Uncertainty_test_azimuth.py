...
for name in minuit_chi2_single.parameters:
    value, error = (
        minuit_chi2_single.values[name],
        minuit_chi2_single.errors[name],
    )
    print(f"Fit value: {name} = {value:.5f} +/- {error:.5f}")
...
chi2_value = (
    minuit_chi2.fval
)  # The value minimised, i.e. Chi2 or -2*LogLikeliHood (LLH) value
chi2_value_single = minuit_chi2_single.fval

# Get number of degrees-of-freedom (Ndof):
N_NotEmptyBin = np.sum(y > 0)

Ndof_value = N_NotEmptyBin - len(
    minuit_chi2.values
)  # ERDA version (older version of Minuit!)
Ndof_value_single = N_NotEmptyBin - len(minuit_chi2_single.values)

Prob_value = stats.chi2.sf(
    chi2_value, Ndof_value
)  # The chi2 probability given N_DOF degrees of freedom
print(
    f"for double gaussian: Chi2 value: {chi2_value:.1f}   Ndof = {Ndof_value:.0f}    Prob(Chi2,Ndof) = {Prob_value:5.3e}"
)

Prob_value_single = stats.chi2.sf(
    chi2_value_single, Ndof_value_single
)  # The chi2 probability given N_DOF degrees of freedom
print(
    f"for single gaussian: Chi2 value: {chi2_value_single:.1f}   Ndof = {Ndof_value_single:.0f}    Prob(Chi2,Ndof) = {Prob_value_single:5.3e}"
)
...
d_single = {
    "N": [minuit_chi2_single.values["N"], minuit_chi2_single.errors["N"]],
    "mu": [minuit_chi2_single.values["mu"], minuit_chi2_single.errors["mu"]],
    "sigma": [
        minuit_chi2_single.values["sigma"],
        minuit_chi2_single.errors["sigma"],
    ],
...
ax.set(
    ...
    title=f"Distribution of z-scores for {target}",
)  # the title of the plot
ax.legend(loc="upper left")
# could also be # loc = 'upper right' e.g.
plt.savefig(
    "/groups/icecube/peter/workspace/graphnetmoon/graphnet/studies/Moon_Pointing_Analysis/plotting/Test_Plots/Model_trained_on_neutrinos/Uncertainty_test/Azimuth_Z_score_double_gaussian.png"
)
...
ax.plot(
    x_axis,
    gaussian(x_axis, *minuit_chi2_single.values),
    "-r",
    label="Chi2 fit single gaussian distribution",
)
...
# could also be # loc = 'upper right' e.g.
plt.savefig(
    "/groups/icecube/peter/workspace/graphnetmoon/graphnet/studies/Moon_Pointing_Analysis/plotting/Test_Plots/Model_trained_on_neutrinos/Uncertainty_test/Azimuth_Z_score_single_gaussian.png"
)
...
plt.legend()
plt.savefig(
    "/groups/icecube/peter/workspace/graphnetmoon/graphnet/studies/Moon_Pointing_Analysis/plotting/Test_Plots/Model_trained_on_neutrinos/Uncertainty_test/Azimuth_Z_score_per_zenith.png"
)
