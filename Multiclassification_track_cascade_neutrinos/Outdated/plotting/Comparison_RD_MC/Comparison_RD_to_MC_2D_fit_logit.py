...
outdir = "/groups/icecube/petersen/GraphNetDatabaseRepository/multi_classification_stop_track_muon/plotting/2d_comparison_logit_"
    ...
    for i in range(len(bins_to_fit)-1):
        for j in range(len(bins_to_fit)-1):
            if counts_RD_to_fit[i,j] > 0:
                ...
                sum+= (counts_RD_to_fit[i,j] - N_scalers[0]*counts_noise_to_fit[i,j] - N_scalers[1]*counts_muon_to_fit[i,j] - N_scalers[2]*counts_neutrino_to_fit[i,j])**2/np.max([counts_noise_to_fit[i,j],counts_muon_to_fit[i,j],counts_neutrino_to_fit[i,j]])
            else:
                sum+=0
    return sum
